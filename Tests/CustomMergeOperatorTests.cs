...
public class CustomMergeOperatorTests
{
    ...
    [TestMethod]
    public void CustomMergeOperatorHandlesFailure()
    {
        var mergeOp = MergeOperators.Create(
            "FailingMerge",
            (ReadOnlySpan<byte> key, MergeOperators.OperandsEnumerator operands, out bool success) =>
        ...
        try
        {
            ...
            // When merge operator returns success=false, RocksDB should throw an exception
            RocksDbException exception = null;
            try
            {
                db.Get("key"u8.ToArray());
                Assert.Fail("Expected RocksDbException to be thrown");
            }
            ...
            Assert.Contains("Merge operator failed", exception.Message);
        }
        finally
        {
            if (Directory.Exists(dbPath))
